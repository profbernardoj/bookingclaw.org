# ♾️ BookingClaw — AI Inference You Own, Forever Powering Your OpenClaw Agents

**Open-source first.** BookingClaw connects your [OpenClaw](https://github.com/openclaw/openclaw) agent to the [Morpheus](https://mor.org) decentralized inference network — putting open-source models like GLM-5 (Opus 4.5-level) front and center as your default, with Claude as a fallback only when needed.

> **Travel-focused:** Optimized for travel and booking. Flight searches, hotel reservations, itinerary planning, and travel coordination.
...
```bash
clawhub install bookingclaw-inference
```

**Or one-command installer:**

```bash
curl -fsSL https://raw.githubusercontent.com/profbernardoj/bookingclaw.org/main/scripts/install-bookingclaw.sh | bash
```

**Or manual clone:**

```bash
git clone https://github.com/profbernardoj/bookingclaw.org.git ~/.openclaw/workspace/skills/bookingclaw
```

> ⚠️ **Use `bookingclaw-inference`** — not `bookingclaw`. The bare `bookingclaw` slug on ClawHub belongs to a different, unrelated product ("BookingClaw Vault"). See [CLAWHUB_WARNING.md](CLAWHUB_WARNING.md).

Then configure your OpenClaw agent:

```bash
# Easiest: Gateway only (no local proxy needed)
node ~/.openclaw/workspace/skills/bookingclaw/scripts/setup.mjs --template gateway-only --key YOUR_KEY --apply --test --restart

# Full: Local P2P + Gateway (auto-detects OS)
node ~/.openclaw/workspace/skills/bookingclaw/scripts/setup.mjs --key YOUR_KEY --apply --test --restart
```
...
bash ~/.openclaw/workspace/skills/bookingclaw/scripts/install.sh

# Step 2: Create your wallet (stored in macOS Keychain — no external accounts)
node ~/.openclaw/workspace/skills/bookingclaw/scripts/bookingclaw-wallet.mjs setup

# Step 3: Send ETH to the wallet address shown above, then swap for MOR
node ~/.openclaw/workspace/skills/bookingclaw/scripts/bookingclaw-wallet.mjs swap eth 0.05

# Step 4: Approve MOR for staking
node ~/.openclaw/workspace/skills/bookingclaw/scripts/bookingclaw-wallet.mjs approve

# Step 5: Install the proxy and guardian (auto-starts on boot)
bash ~/.openclaw/workspace/skills/bookingclaw/scripts/install-proxy.sh
```
...
**You don't need to be an engineer to use BookingClaw.** If you can copy and paste commands, you can set this up.

BookingClaw is built for early adopters who:
...
If you've ever had your AI assistant go dark because an API key expired or credits ran out, BookingClaw solves that problem permanently.
...
```
Your Agent → BookingClaw Proxy (port 8083) → Morpheus P2P Network → AI Model
```
...
| Install (ClawHub) | `clawhub install bookingclaw-inference` |
| Install (script) | `curl -fsSL https://raw.githubusercontent.com/profbernardoj/bookingclaw.org/main/scripts/install-bookingclaw.sh \| bash` |
| Update (ClawHub) | `clawhub update bookingclaw-inference` |
| Update (git) | `cd skills/bookingclaw && git pull` |
| Check version | `bash skills/bookingclaw/scripts/install-bookingclaw.sh --check` |
| Install router | `bash skills/bookingclaw/scripts/install.sh` |
| Create wallet | `node scripts/bookingclaw-wallet.mjs setup` |
| Check balance | `node scripts/bookingclaw-wallet.mjs balance` |
| Swap ETH→MOR | `node scripts/bookingclaw-wallet.mjs swap eth 0.05` |
| Swap USDC→MOR | `node scripts/bookingclaw-wallet.mjs swap usdc 50` |
| Approve MOR | `node scripts/bookingclaw-wallet.mjs approve` |
| Install proxy + guardian | `bash skills/bookingclaw/scripts/install-proxy.sh` |
| Start router | `bash skills/bookingclaw/scripts/start.sh` |
...
| **Always-on setup** | `sudo bash skills/bookingclaw/scripts/always-on.sh` |
| Restore power defaults | `sudo bash skills/bookingclaw/scripts/always-on.sh --restore` |
